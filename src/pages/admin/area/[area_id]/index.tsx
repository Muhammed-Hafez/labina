...
import { AreaForm, IAreaPost } from "@/lib/@types/Area";
import {
  Select,
  Checkbox,
  TextField,
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  try {
    const res = await httpAdminSSR(req).get("/area/" + query.area_id);
    props.area = await res.data.data;
  } catch (error) {}
...
function EditArea(props: IAreaProps) {
  ...
  const [filteredCityList, setFilteredCityList] = useState<ICityPost[]>(
    props.area?.country
      ? props.cityList.filter((x) => x.country === props.area?.country)
      : []
  );
  const [selectedCountry, setSelectedCountry] = useState<ICountryPost | null>(
    props.countryList.find((x) => x._id === props.area?.country) || null
  );
  const [selectedCity, setSelectedCity] = useState<ICityPost | null>(
    props.cityList.find((x) => x._id === props.area?.city) || null
  );
  ...
  const {
    ...
    trigger,
    formState: { errors },
  } = useForm<AreaForm>({ values: props.area });
  ...
  useEffect(() => {
    const title =
      props.area?.titleList?.find((x) => x.lang === selectedLanguage)?.value ??
      "";
  ...
  const onSubmit: SubmitHandler<AreaForm> = (values: AreaForm) => {
    ...
    const titleList = props.area?.titleList ?? [];
