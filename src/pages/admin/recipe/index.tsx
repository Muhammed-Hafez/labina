...
function BrowseRecipe(props: IRecipeProps) {
  const [open, setOpen] = useState(false);
  const [deletedItemId, setDeletedItemId] = useState("");
  const [recipeList, setRecipeList] = useState<IRecipeGet[]>([]);
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell(params) {
        return (
          <img
            src={"/api/static/images/recipe/" + params.row.image}
            alt="recipe img"
            height="50"
          />
        );
        ...
        return (
          <>
            {allowedActions.includes("edit") && (
              <ButtonBase
                href={"/admin/recipe/" + params.row._id}
                LinkComponent={Link}
                className="icon__btn"
              >
      ...
      {allowedActions.includes("delete") && (
        ...
        >
          <DialogTitle id="delete-modal-title">
            {t(`Are you sure to delete this recipe?`)}
          </DialogTitle>
        ...
        <div className={styles["container__header"]}>
          ...
          {allowedActions.includes("add") && (
            <div className={styles["actions"]}>
              <Link href={"/admin/recipe/new"} className="btn__contained">
                {t(`Add Recipe`)}
              </Link>
            </div>
          )}
