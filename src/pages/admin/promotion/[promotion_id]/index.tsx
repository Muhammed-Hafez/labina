...
import { PromotionForm, IPromotionGet } from "@/lib/@types/Promotion";
import {
  Checkbox,
  TextField,
  FormControlLabel,
...
function EditPromotion(props: IPromotionProps) {
  const { locale, defaultLocale, locales, push, query } = useRouter();
  ...
  useEffect(() => {
    httpAdmin
      .get("/promotion/f/" + query.promotion_id)
      .then((res) => {
        const foundPromotion = res.data.data;
        setSelectedCategoryList(foundPromotion.categoryList ?? []);
        setSelectedProductList(foundPromotion.productList ?? []);
        setStartDate(foundPromotion.start ? dayjs(foundPromotion.start) : null);
        setEndDate(foundPromotion.end ? dayjs(foundPromotion.end) : null);
        reset({
          ...foundPromotion,
          categoryList: foundPromotion?.categoryList?.map((x: any) => x._id),
          productList: foundPromotion?.productList?.map((x: any) => x._id),
        });
      })
      .catch((err) => {});
  }, []);
