...
import { IRoleGet, RoleForm } from "@/lib/@types/Role";
...
function EditRole() {
  const [Role, setRole] = useState<IRoleGet>();
  ...
  useEffect(() => {
    async function getData() {
      try {
        const res = await httpAdmin.get("/role/" + query.role_id);
        setRole(await res.data.data);
        reset(new RoleForm(res.data.data));
        setKey((prev) => prev + 1);
      } catch (error) {}
  ...
  const onSubmit: SubmitHandler<RoleForm> = (values: RoleForm) => {
    ...
    httpAdmin
      .patch("/role", { ...values, permissionList })
      .then((res) => {
        push("/admin/role");
      })
      .catch((err) => {
