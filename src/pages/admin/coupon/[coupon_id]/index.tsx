...
import { CouponForm, ICouponGet } from "@/lib/@types/Coupon";
import {
  Checkbox,
  TextField,
  FormControlLabel,
...
function EditCoupon(props: ICouponProps) {
  ...
  useEffect(() => {
    httpAdmin
      .get("/coupon/f/" + query.coupon_id)
      .then((res) => {
        const foundCoupon = res.data.data;
        setSelectedCategoryList(foundCoupon.categoryList ?? []);
        setSelectedProductList(foundCoupon.productList ?? []);
        setStartDate(foundCoupon.start ? dayjs(foundCoupon.start) : null);
        setEndDate(foundCoupon.end ? dayjs(foundCoupon.end) : null);
        reset({
          ...foundCoupon,
          categoryList: foundCoupon?.categoryList?.map((x: any) => x._id),
          productList: foundCoupon?.productList?.map((x: any) => x._id),
        });
        setKey((prev) => prev + 1);
      })
      .catch((err) => {});
