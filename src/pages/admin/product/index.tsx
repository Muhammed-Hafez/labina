...
function BrowseProduct(props: IProductProps) {
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      valueGetter: (params: GridValueGetterParams) =>
        `${getAutocompleteLabel(
          params.row.category.titleList,
          locale,
          defaultLocale
        )}`,
    },
    { field: "slug", headerName: `${t("Slug")}` },
    { field: "sku", headerName: `${t("SKU")}` },
    { field: "quantity", headerName: `${t("Quantity")}`, type: "number" },
    { field: "mainPrice", headerName: `${t("Price")}`, type: "number" },
    {
      field: "mainImage",
      headerName: `${t("Main Image")}`,
      sortable: false,
      renderCell(params) {
        return (
          <img
            src={"/api/static/images/product/" + params.row.mainImage}
            alt="product"
            height="50"
          />
        );
    ...
    {
      field: "isNewProduct",
      headerName: `${t("New Product")}`,
      type: "boolean",
    },
    {
      field: "isFeaturedProduct",
      headerName: `${t("Featured Product")}`,
      type: "boolean",
    },
    {
      field: "available",
        ...
        <div className={styles["container__header"]}>
          ...
          {allowedActions.includes("add") && (
            <div className={styles["actions"]}>
              <Link href={"/admin/product/new"} className="btn__contained">
                {t(`Add Product`)}
              </Link>
            </div>
          )}
